...
class _StorySelectionPageState extends State<StorySelectionPage> {
  Future<List<Map<String, Object>>> getStories() async => await database.query('stories');

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(child: Icon(Icons.add), onPressed: () => Navigator.of(context).pushNamed('/storyBuilder')),
        body: Column(children: <Widget>[
          Padding(padding: EdgeInsets.only(left: 50, top: 50, right: 50),
              child: Row(children: <Widget>[
                ...
                Expanded(child: Center(child: Text('Story Archive', style: TextStyle(fontSize: 24))))
              ])),
          Expanded(child: FutureBuilder<List<Map<String, Object>>>(future: getStories(),
                builder: (context, snapshot) => snapshot.hasData?
            ListView.separated(itemCount: snapshot.data.length, itemBuilder: (context, index) => FlatButton(
                onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => StoryBuilderPage(Map.from(snapshot.data[index]))
                )),
