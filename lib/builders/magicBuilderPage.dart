...
class _MagicBuilderPageState extends State<MagicBuilderPage> {
      ...
      Step(
          ...
          content: Padding(
              ...
              child: Wrap(runSpacing: 20, children: <Widget>[
                ...
                textField(binding: 'limits', label: 'What are the limits of magic?'),
                textField(binding: 'consequences', label: 'What are the consequences of magic?'),
      ...
      Step(
          ...
          content: Padding(
              ...
              child: Wrap(runSpacing: 20, children: <Widget>[
                ...
                textField(binding: 'whyKnows', label: 'Why do they know about magic?'),
                textField(binding: 'culture', label: 'What is the culture created by magic?'),
                textField(binding: 'governance', label: 'How is magic governed?')
              ]))),
      Step(
          ...
          content: Padding(
              padding: EdgeInsets.symmetric(vertical: 5),
              child: Wrap(runSpacing: 20, children: <Widget>[
                textField(binding: 'called', label: 'In this world, magic is called'),
                textField(binding: 'base', label: 'And is based on'),
                textField(binding: 'comesFrom', label: 'Magic comes from'),
                textField(binding: 'practitioners', label: 'practitioners are called'),
                ...
                textField(binding: 'timeline', label: 'Magic has been around since'),
                textField(binding: 'awareness', label: 'The people who know about magic are'),
        ...
        body: Column(children: <Widget>[
          Padding(padding: EdgeInsets.only(left: 50, top: 50, right: 50),
              child: Row(children: <Widget>[
                ...
                Expanded(child: Center(child: Text('Magic Builder', style: TextStyle(fontSize: 24))))
              ])),      Expanded(
          child: Form(
              key: _formKey,
              child: Stepper(
  ...
  saveData() async {
    if (widget.data['id'] == null)
      await database.insert('magic', widget.data..['id'] = Random().nextInt(1000000), conflictAlgorithm: ConflictAlgorithm.replace);
    else
      await database.update('magic', widget.data, where: 'id = ?', whereArgs: [widget.data['id']]);
  }
