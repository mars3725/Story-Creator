...
class _TechnologyBuilderPageState extends State<TechnologyBuilderPage> {
    ...
    steps = <Step>[
          ...
          content: Padding(
              ...
              child: Wrap(runSpacing: 20, children: <Widget>[
                textField(
                    ...
                    tooltip: "This is what people call Technology. If it has multiple names then this is where you put them. Naming Technology can be influenced by the language of the world, or what it does."),
                textField(binding: 'basedOn', label: 'What is technology based on?'),
                ...
                textField(binding: 'limits', label: 'What are the limits/costs of technology?'),
                textField(binding: 'consequences', label: 'What are the consequences of technology?'),
      ...
      Step(
          ...
          content: Padding(
              padding: EdgeInsets.symmetric(vertical: 5),
              child: Wrap(runSpacing: 20, children: <Widget>[
                textField(binding: 'beginning', label: 'How did technology begin?'),
                ...
                textField(binding: 'why', label: 'Why can they use technology?'),
                textField(binding: 'whoKnows', label: 'Who knows about technology?'),
                textField(binding: 'whyKnows', label: 'Why do they know about technology?'),
                textField(binding: 'culture', label: 'What is the culture created by technology?'),
                textField(binding: 'governance', label: 'How is the technology governed?')
              ]))),
      Step(
          ...
          content: Padding(
              padding: EdgeInsets.symmetric(vertical: 5),
              child: Wrap(runSpacing: 20, children: <Widget>[
                textField(binding: 'called', label: 'In this world, tech is called'),
                ...
                textField(binding: 'timeline', label: 'Tech has been around since'),
                textField(binding: 'comesFrom', label: 'And comes from'),
                textField(binding: 'practitioners', label: 'It can be used by'),
                textField(binding: 'awareness', label: 'Those who know about tech are'),
  ...
  saveData() async {
    if (widget.data['id'] == null)
      await database.insert('technologies', widget.data..['id'] = Random().nextInt(1000000), conflictAlgorithm: ConflictAlgorithm.replace);
    else
      await database.update('technologies', widget.data, where: 'id = ?', whereArgs: [widget.data['id']]);
  }
